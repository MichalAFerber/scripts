...
update_docker_images() {
      ...
      if [[ -n "$projects" ]]; then
        ...
        if ((${#names[@]})); then
          for p in "${names[@]}"; do
            ...
            $DCMD -p "$p" pull || warn "Pull failed for $p"
            $DCMD -p "$p" up -d || warn "Up failed for $p"
          done
        else
  ...
  if [[ -n "$images" ]]; then
    ...
    for c in "${cnames[@]}"; do
      ...
      docker restart "$c" || warn "Restart failed for $c"
    done
  else
...
prune_docker() {
  ...
  docker image prune -af || warn "Image prune failed"
...
restart_docker_containers() {
  ...
  if ((${#cnames[@]})); then
    for c in "${cnames[@]}"; do
      echo "Restarting $c"
      docker restart "$c" || warn "Restart failed: $c"
    done
  else
...
maybe_reboot() {
  ...
  if [[ "$REBOOT_IF_NEEDED" == "1" && $need -eq 1 ]]; then
    ...
    /sbin/reboot || systemctl reboot || shutdown -r now || true
  elif [[ $need -eq 1 ]]; then
