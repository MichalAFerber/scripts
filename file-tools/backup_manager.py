...
Usage:
    python backup_manager.py <source_dir> <backup_dir> [--dry-run]
...
Example:
    ...
    python backup_manager.py ~/Documents ~/Backups/Documents --dry-run
...
class BackupManager:
    def __init__(self, source_dir, backup_dir, max_backups=10, dry_run=False):
    ...
    def restore_backup(self, backup_index=-1, restore_dir=None):
        ...
        if self.dry_run:
            print(f"\n[DRY RUN] Would restore backup: {backup['filename']}")
            print(f"[DRY RUN] To directory: {restore_dir}")
            print(f"[DRY RUN] Files: {backup['files_count']}")
            return True
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Smart incremental backup manager with compression and version management."
    )
    parser.add_argument("source_dir", help="Source directory to back up")
    parser.add_argument("backup_dir", help="Directory to store backups")
    parser.add_argument("--max-backups", type=int, default=10, help="Maximum backup versions to keep (default: 10)")
    parser.add_argument("--dry-run", action="store_true", help="Preview changes without creating/restoring backups")
    args = parser.parse_args()

    manager = BackupManager(args.source_dir, args.backup_dir,
                           max_backups=args.max_backups, dry_run=args.dry_run)
